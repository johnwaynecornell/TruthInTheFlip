...
def parse_args():
    ...
    parser.add_argument(        "--control-block-size",
        type = int,
        default = 4,
        help = "Number of consecutive 10B segments preserved inside each permuted control block.",
    )
...
def permute_blocks(
    ...
    if block_size <= 0:
        raise ValueError(
            "control block size must be greater than zero"
        )
...
def block_permutation_control(
    ...
    return {
        ...
        "BlockControlBaselineMeanZ": float(
            np.mean(baseline_null)
        ),

        "BlockControlBaselineStdZ": float(
            np.std(
                baseline_null,
                ddof=1,
            )
        ...
        "BlockControlBaseline95PctZ": float(
            np.quantile(
                baseline_null,
                0.95,
            )
        ...
        "BlockControlCandidateMeanZ": float(
            np.mean(candidate_null)
        ),

        "BlockControlCandidateStdZ": float(
            np.std(
                candidate_null,
                ddof=1,
            )
        ...
        "BlockControlCandidate95PctZ": float(
            np.quantile(
                candidate_null,
                0.95,
            )
        ...
        "BlockControlDeltaMean": float(
            np.mean(delta_null)
        ),

        "BlockControlDeltaStd": float(
            np.std(
                delta_null,
                ddof=1,
            )
...
def main():
    ...
    #
    # Quant2 was intentionally absent during methodology development.
    #
    # It is introduced here only after the persistence rule,
    # walk-forward design, and structural control were frozen,
    # as the prospective validation tracker.
    #
    trackers = {
    ...
    for tracker_name, filename in trackers.items():
        ...
        for candidate in CANDIDATES:
            ...
            control = block_permutation_control(
                folds = folds,
